...
?>
    ...
    var date1 = $("#date1").val();
    var date2 = $("#date2").val();
    var url = "<?php echo base_url('index.php/PIR/getAllcanceled/'); ?>";
    
    // Show loading indicator
    $("#tableData").html('<div class="loading">Loading...</div>');

    $.post(url, {
        "date1": date1,
        "date2": date2
    }, function(data) {
        if (data && data.length > 0) { // Check if data exists and has length greater than 0
            let html = '';
            let total = 0;
            ...
            data.forEach((element) => {
                html += `<tr>
                            ...
                            <td>${element.ReceiveDate}</td>
                        </tr>`;
                total += parseFloat(element.KitQty);
            });
            html += `</tbody></table>`;
            html += `<div>Total canceled quantity: ${total.toFixed(2)}</div>`;

            $("#tableData").html(html);

            // Initialize the DataTable here as before
            $('#Data').dataTable({
                // DataTable initialization options
            });
        } else {
            // If no data is available, display a message
            $("#tableData").html('<div class="alert alert-warning" role="alert">Data is not available for the selected range.</div>');
        }
    }).fail(function() {
        // Handle request failures
        $("#tableData").html('<div class="alert alert-danger" role="alert">Failed to load data. Please try again later.</div>');
    });
}
