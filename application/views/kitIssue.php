                        ...
                        <div>
                            <input type="password" class="form-control input-lg" name="password">
								...
								<div id="exampleModalEditMat" class="panel">
									<div class="panel-hdr">
										<h2>
											Printed <span class="fw-300"><i>Label Issuance</i></span>
										</h2>
...
<script src="<?php echo base_url(); ?>assets/js/datagrid/datatables/datatables.export.js"></script>
    ...
    var url = "<?php echo base_url('index.php/Kitsissuance/getkitsissuance1/') ?>" + date1 + "/" + date2;
    ...
    $.get(url, function(data) {
        ...
        var table = '';
        if (data['getkitsissuance'].length > 0) {
            // Data is available
            table += `
            <div class="row">
            ...
            data['getkitsissuance'].forEach(e => {
                table += ` <tr>
                                <td>${formatDate(e.PlanDate)}</td>
                                ...
                                <td>${e.Issuedby}</td>
                                <td>${e.Type}</td>
                                <td>${formatDate(e.PrintDate)}</td>
                                <td>${e.IssueDate ? e.IssueDate : 'Pending' }</td>
                                <td>${e.Days ? e.Days : 0 }</td>
                                <td>${e.KeyNum}</td>
                                <td>${e.Receivedby ? e.Receivedby : 'Pending' }</td>
                            </tr>`;
            });
...
// // when end date is disabled
// url = "<?php echo base_url("index.php/Kitsissuance/getkitsissuance1/") ?>" + date1 + "/" + date2 
// // $.get(url, function(data) {
